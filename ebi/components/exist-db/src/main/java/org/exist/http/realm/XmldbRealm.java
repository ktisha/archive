...
public class XmldbRealm extends org.apache.catalina.realm.RealmBase {
    ...
    private static Log log = LogFactory.getLog(XmldbRealm.class);
	...
	public Principal authenticate(String username, String credentials) {
		...
		if (validated) {
			if (containerLog.isTraceEnabled())
				containerLog.trace(sm.getString("userDatabaseRealm.authenticateSuccess", username));
			return (principal);
		} else {
			if (containerLog.isTraceEnabled())
				containerLog.trace(sm.getString("userDatabaseRealm.authenticateFailure", username));
			return (null);
		}
	}
	/**
		...
		try {
			...
			if(admin){
				...
				if(adminUser.getPassword() == null){
					...
					log.info("Update Admin User on inital start");
				}
			}
