...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="ae-contents">
        ...
        <xsl:variable name="vFilteredExperiments" select="/experiments/experiment"/>
    ...
    <xsl:template match="experiment">
        ...
        <tr>
            ...
            <td class="col_name">
                ...
                <xsl:if test="count(similarExperiments) != 0">
                    <div class="similar_experiments">
                        Similar experiments:<br/>
                        <xsl:apply-templates select="similarExperiments"/>
                    </div>
                </xsl:if>
            </td>
    ...
    <xsl:template name="data-files-main">
        ...
        <xsl:choose>
            <xsl:when test="true()"><img src="{$basepath}/assets/images/silk_tick.gif" width="16" height="16" alt="*"/></xsl:when>
            <xsl:otherwise><img src="{$basepath}/assets/images/silk_data_unavail.gif" width="16" height="16" alt="-"/></xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template name="similarExperiments" match="similarExperiments">
        <xsl:for-each select="./similarExperiment">
            <a href="{$basepath}/experiments/{accession}">
                <xsl:apply-templates select="./accession" mode="copy" />
            </a>&#xa0;
        </xsl:for-each>
