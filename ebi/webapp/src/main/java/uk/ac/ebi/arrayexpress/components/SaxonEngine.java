...
public class SaxonEngine extends ApplicationComponent implements URIResolver, ErrorListener
{
    ...
    public String transformToString( String srcXmlString, String stylesheet, Map<String,String[]> params ) throws Exception
    {
        String str;
        ByteArrayOutputStream outStream = new ByteArrayOutputStream();

        Source src = new StreamSource(new StringReader(srcXmlString));
        if (transform(src, stylesheet, params, new StreamResult(outStream))) {
            str = outStream.toString(XML_STRING_ENCODING);
            return str;
        } else {
            return null;
